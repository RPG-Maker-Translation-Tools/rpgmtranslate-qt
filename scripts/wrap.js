...
/**
 * Word-wrap a single column to a max line length.
 *
...
function rpgmLineCallback(sourceText, translations, filename, lineNumber, options) {
    ...
    if (columns.length < options.columnIndex) {
        return columns.map(() => undefined);
    }

    const wrapLength = options.wrapLength;
    const words = columns[options.columnIndex].split(" ");

    let wrapped = "";
    let lineLen = 0;

    for (const word of words) {
        if (lineLen > 0 && lineLen + 1 + word.length > wrapLength) {
            wrapped += "\n";
            lineLen = 0;
        } else if (lineLen > 0) {
            wrapped += " ";
    ...
    const result = new Array(columns.length).fill(undefined);
    result[options.columnIndex] = wrapped;
    return result;
}
